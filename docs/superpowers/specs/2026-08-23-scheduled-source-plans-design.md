...
后续逐题审计确认：十个 URL 只有九个内容簇，其中两篇字节帖子是同一套 17 题的短版与扩写版；现有 552 个面试问题标题对真实样本归并出的 72 个主题簇只直接覆盖 12 个、部分覆盖 5 个、完全缺失 55 个。因此问题指纹去重和 `covered/evolved/new` 差距报告是正式验收项，不是可选增强。
...
> 2026-08-28 修订：本节关于“只看星主”的固定回看窗口、单视图条数上限和“不做历史全量”的约束，已由 [知识星球「只看星主」历史审计与逐日账本设计](./2026-08-28-zsxq-owner-history-ledger-design.md) 替代。“最新 / 精华”仍走有界核对；“只看星主”改为默认逐日账本和显式、可恢复的 `owner-history` 模式。
