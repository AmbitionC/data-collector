...
- 牛客交付基准：每个外部终态批次写入 `<configDir>/benchmarks/<batch-id>.json`。单元测试校验批次/任务真实时间戳计算出的活跃耗时与任务 P50/P90、0600 原子落盘、配置并发上限与实测峰值分离，以及报告不会包含 URL、正文、HTML、作者、Cookie、错误消息或本机证据路径。若基准写入失败，批次必须转为“需处理”，已保存证据和交付 ID 保持不变。
- 端到端交付冒烟：用两个隔离收件箱验证精确批次筛选、成功项消费、阻塞项/运营候选保留，并运行真实扩展标签页注册器，要求模拟终态自动标签页为 0。
...
1. 运行 `npm run package`，确认稳定安装目录和 `data-collector-extension-0.4.25.zip` 均生成。
...
打包脚本从已校验的 `packages/extension/dist/manifest.json` 读取版本，先在临时目录准备完整 ZIP 和已解压目录，再切换稳定安装目录。只有 0.4.25 成品验证完成后才删除旧 ZIP；历史包清理失败只告警，不会把已经提交的新成品误报成构建失败。

```bash
npm run package
shasum -a 256 artifacts/data-collector-extension-0.4.25.zip
npm run package
shasum -a 256 artifacts/data-collector-extension-0.4.25.zip
```
