...
1. 运行 `npm run package`，确认稳定安装目录和 `data-collector-extension-0.4.22.zip` 均生成。
...
10. `unzip -l artifacts/data-collector-extension-0.4.22.zip` 只显示八个允许文件。
...
打包脚本从已校验的 `packages/extension/dist/manifest.json` 读取版本，先在临时目录准备完整 ZIP 和已解压目录，再切换稳定安装目录。只有 0.4.22 成品验证完成后才删除旧 ZIP。

```bash
npm run package
shasum -a 256 artifacts/data-collector-extension-0.4.22.zip
npm run package
shasum -a 256 artifacts/data-collector-extension-0.4.22.zip
```
