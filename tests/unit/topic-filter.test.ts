...
describe('楼市与相亲', () => {
  ...
  it('完整正文复筛实机漏判的券商开户、财新促销与保险导流', () => {
    const posts = [
      '说个事情，国金证券已经取消了免五政策。目前还剩华宝和海通有免五政策，之前的券商开户文章可参考。',
      '最近不少人问起免五的券商开户还有没有，想低费率开户的可以参看此帖。',
      '双11还有几个小时就结束了，财新产品的降价只剩最后几个小时，需要续费或购买的可参见原帖。',
      '提醒下，今天是双11，财新降价的最后一天。如有需要的，可参见之前的发帖来购买或续费，价格是138元。',
      '提醒一下，上个月在星球里分享的3.5%年化利率港险，目前额度已消耗70%~80%，保险公司将于4月17日下架产品，如果有需要可以尽早联系。',
    ];
    for (const post of posts) expect(excludedBy(post)?.label).toBe('推广/带货内容');
  });

  it('过滤以限时降门槛、折扣和预约咨询导流的保险推广', () => {
    const post = '说个事，向关心港险的小伙伴分享一个友邦的产品，这款产品是面向高净值，'
      + '门槛原本是50w美元，截止9月30号的门槛为25w美元起。'.repeat(2)
      + '这款产品目前最低折扣都有23%，截止本月30号门槛减半。'
      + '有想法的同学，可点下面的链接预约咨询，预约后等待经纪人联系你就行。';
    expect(excludedBy(post)?.label).toBe('推广/带货内容');
  });

  it('不误伤只讨论保险门槛和配置方法的知识正文', () => {
    const post = '保险配置最重要的是先核对家庭现金流与保障缺口。'
      + '高保额产品通常有投保门槛，但本文不推荐具体产品，也不提供咨询或购买渠道。';
    expect(excludedBy(post)).toBeUndefined();
  });

  it('把 REIT 发行认购建议视为打新内容，不误伤普通 REIT 分析', () => {
    expect(excludedBy(
      '今天有一个reits认购，华泰苏州恒泰租赁住房REIT（508085），可以认购。',
    )?.label).toBe('打新内容');
    expect(excludedBy(
      '这只 REIT 的底层资产是租赁住房，本文只分析出租率、现金流和估值。',
    )).toBeUndefined();
  });
