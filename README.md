...
用户明确不看的类别在采集端就跳过，目前内置**打新（新股申购）**。判据要求「强信号 + 至少两个不同信号」，
避免把只是顺口提到「破发」的正常投资分析误伤。被跳过的条目**照样出现在「本轮明细」里**，
状态是已跳过、原因写明类别 —— 绝不静默丢弃，你随时能看到自己漏掉了什么。规则在
`packages/extension/src/topicFilter.ts`。

其余选题偏好（例如不关注楼市）仍留在归档侧处理。
