...
- 每条帖子按各自的 `/topic/<帖子号>` 地址建独立条目，不会相互覆盖；
- 采完一屏会把已处理的帖子从版面收起、滚动加载下一批，直到没有新内容（单次上限 60 条 / 12 轮）；
- 拿不到自身链接的帖子如实计入“已跳过”，不会用列表页地址凑数；
- 面板实时显示“已入库 / 已跳过 / 失败”，结束后可以“继续采下一批”。

选题过滤（例如不关注楼市）留在归档侧处理，采集端不做内容过滤。
