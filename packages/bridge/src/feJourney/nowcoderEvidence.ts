...
const FIRST_HAND_PATTERN = /(?:^|[，。；：\s])(?:我|本人)(?:参加|面试|投递|做过|的|先|主要|当时)|面试官|面试时间|面试经过|我的(?:项目|经历|回答)/u;
...
export function analyzeNowcoderEvidence(document: CollectedDocument): NowcoderEvidence {
  ...
  const firstHand = FIRST_HAND_PATTERN.test(combined);
  const compilation = COMPILATION_PATTERN.test(title) || !firstHand && COMPILATION_PATTERN.test(combined);
