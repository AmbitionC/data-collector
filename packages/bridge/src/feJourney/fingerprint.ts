...
const INTERVIEW_TOPIC_RULES: readonly [string, RegExp][] = [
  ...
  ['business-metrics', /(?:实习|业务).{0,16}(?:指标|数据)|(?:指标|数据).{0,16}(?:实习|业务)/iu],
  ['business-context', /项目.{0,12}(?:背景|上下游|业务链路)|(?:背景|上下游|业务链路).{0,12}项目/iu],
  ...
  ['agent-application', /Agent.{0,16}(?:场景|解决|应用)|(?:场景|解决|应用).{0,16}Agent/iu],
  ['agent-architecture', /(?:理解|设计).{0,8}Agent|Agent.{0,12}(?:系统|架构|核心模块)/iu],
  ['agent-framework', /Agent.{0,16}(?:框架|代码架构)|(?:框架|代码架构).{0,16}Agent/iu],
  ['openmanus', /OpenManus/iu],
  ['harness', /Harness/iu],
  ...
  ['agent-concurrency', /Agent.{0,16}(?:并发|限流)|(?:并发|限流).{0,16}Agent/iu],
  ['planner-executor-critic', /Planner.{0,16}Executor.{0,16}Critic/iu],
  ['context-management', /上下文.{0,16}(?:压缩|拆分|过长|超长|管理|处理)/iu],
  ['redis-structure', /Redis.{0,12}(?:结构|数据结构)/iu],
  ['jvm-classloading', /(?:Java|JVM|Class).{0,16}(?:类加载|加载过程)|类加载机制/iu],
  ['agent-ad-optimization', /Agent.{0,24}广告投放|广告投放.{0,24}Agent/iu],
